...
class NAM(BaseMethod):
    ...
    def attack(self, x, y=0):
        ...
        self.model.eval()
        if self.is_target:
            x_adv,pertubation = self._attackWithTarget(x, self.target, self.epoch, self.eps, self.beta1, self.beta2)
    ...
    def _attackWithTarget(self, x, target, epoch, eps, beta1, beta2):
