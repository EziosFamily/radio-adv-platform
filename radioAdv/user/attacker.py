...
class Attacker(object):
    ...
    def attack_set(self, model, data_loader):
        ...
        for idx,(x,y, x_snr) in enumerate(tqdm(data_loader)):
            ...
            # x_norm = torch.norm(x, p=2, dim=(1, 2, 3))
            # 计算扰动的 L2 范数
            # 如果 pertubations 已经被转换为 numpy 数组，确保它转换回 PyTorch 张量
            if isinstance(pertubations, np.ndarray):
                pertubations = torch.tensor(pertubations).to(x.device)  # 确保转换回张量并保持在正确的设备上
            perturbation_norm = torch.norm(pertubations, p=2, dim=(1, 2, 3))
            # perturbation_norm = torch.norm(pertubations, p=2, dim=(1, 2, 3))
            # 计算每个样本的变化率
            change_ratios = perturbation_norm / x_norm / 3
            # 打印每个样本的变化率
            # print(f"Sample change ratios: {change_ratios.cpu().numpy()}")
            all_change_ratios.append(change_ratios.cpu().numpy())


        # 合并所有批次的变化率，并计算平均变化率
        all_change_ratios = np.hstack(all_change_ratios)
        mean_change_ratio = np.mean(all_change_ratios)
        ...
        self.plot_confusion_matrix_figure(self.figure_dir, predict.reshape(predict.shape[0]), targets.reshape(targets.shape[0]), self.mods)
        # 记录攻击结果
        ...
        log['snr_acc'] = snr_acc
        log['mean_change_ratio'] = mean_change_ratio  # 将平均变化率也记录在日志中

        return log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio

    def start_attack(self, data_loader):
        attack_method = self.config.Switch_Method['method']
        log = {}
        if attack_method == 'White_Attack':
            log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio = self.white_attack(data_loader)
        elif attack_method == 'Black_Attack':
            log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio = self.black_attack(data_loader, **getattr(self.config, attack_method))
        elif attack_method == 'Shifting_Attack':
            log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio = self.shifting_attack(data_loader, **getattr(self.config, attack_method))


        return log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio

    def white_attack(self, data_loader):
        log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio = self.attack_set(self.model, data_loader)
        return log, real_sample, adv_sample, acc, predict, targets, all_change_ratios, x_snrs, mean_change_ratio
    ...
    def black_attack(self, data_loader, threat_model, black_model, is_uap, eps):
        ...
        white_log, real_sample, adv_sample, adv_pertub, predicts, targets, logits, x_snrs, mean_change_ratio = self.attack_set(white_model, data_loader)
        ...
        if is_uap:
            ...
            adv_sample = adv_sample.astype(float)
        ...
        # target_index = targets >= 8
        # adv_sample = adv_sample[target_index]
        ...
        return log, real_sample, adv_sample, adv_pertub, predicts, targets, logits, x_snrs, mean_change_ratio
    ...
    def shifting_attack(self, data_loader, load_parameter, parameter_path, is_save_parameter, shift_k, is_uap, eps, is_sim, save_k):
        ...
        print(f"load_parameter: {load_parameter} ")
        #  进行白盒攻击， 得到对抗样本
    ...
    def _model_test(self, model, data_loader):
        ...
        self.plot_confusion_matrix_figure(self.figure_dir, predict.argmax(1), targets, self.mods)
    ...
    def pick_data(self, data_loader):
        ...
        """
        ## RML2018是(24, 26) * 10 , RML2016是 (11, 20) * 10,  (类别数目，SNR数目) * 采样数目/类别，SNR
        sample_matrix = np.ones((24, 26)) * 10
        data = []
        labels = []
        snrs = []
        for idx,(x,y, x_snr) in enumerate(tqdm(data_loader)):
        ...
        dirname = "/home/yuzhen/wireless/RML2018.01a"
        pickle.dump([data, labels, snrs], open(os.path.join(dirname, 'attack_data_rml2018.p'), 'wb'))
        print('-----已完成数据存储-----')
