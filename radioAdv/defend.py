...
from utils import *
from local_paths import dataloader_workers, build_dataset_kwargs
import argparse
import yaml
...
if __name__ == '__main__':
    ...
    dataset_kwargs = build_dataset_kwargs(getattr(config, config.CONFIG['dataset_name']))
    train_dataset = getattr(module_loader, config.CONFIG['dataset_name']+'TrainSet')(**dataset_kwargs)    
    workers = dataloader_workers()
    train_loader = DataLoader(train_dataset, batch_size = config.ARG['batch_size'], shuffle=True, num_workers=workers)
    test_dataset = getattr(module_loader, config.CONFIG['dataset_name']+'TestSet')(**dataset_kwargs)  
    snrs, mods = test_dataset.get_snr_and_mod()  
    test_loader = DataLoader(test_dataset, batch_size = config.ARG['batch_size'], shuffle=False, num_workers=workers)
    ...
    defender_name = config.CONFIG['defender_name']
    defender_kwargs = getattr(config, defender_name, {}) if hasattr(config, defender_name) else {}
    if not isinstance(defender_kwargs, dict):
        defender_kwargs = {}
    trainer = getattr(module_defender, defender_name)(model, train_loader, test_loader, criterion, optimizer, metrics, config, snrs = snrs, mods = mods, **defender_kwargs)
    print('start train')
    trainer.train()
